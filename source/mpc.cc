...
char *mpc_err_string(mpc_err_t *x) {
  ...
  if (x->expected_num >= 2) {
    ...
    mpc_err_string_cat(buffer, &pos, &max, "%s _or %s", 
...
static mpc_err_t *mpc_err_repeat(mpc_err_t *x, const char *prefix) {
  ...
  if (x->expected_num > 1) {
    ...
    expect = (char *) realloc(expect, strlen(expect) + strlen(x->expected[x->expected_num-2]) + strlen(" _or ") + 1);
...
static mpc_err_t *mpc_err_many1(mpc_err_t *x) {
  return mpc_err_repeat(x, "one _or more of ");
}
...
** operation: String, File _and Pipe.
**
** String is easy. The whole contents are 
** loaded into a buffer _and scanned through.
...
** one. As we assume pipes cannot be seeked - _and 
...
** backtracking _and make LL(1) grammars easy
...
typedef union {
  ...
  mpc_pdata_not_t _not;
  mpc_pdata_repeat_t repeat;
  mpc_pdata_and_t _and;
  mpc_pdata_or_t _or;
} mpc_pdata_t;
...
** concisely matches some construct _or pattern.
**
** Particularly nice are the `_or` _and `_and`
** types which have a broken but mirrored structure
** with return value _and error reflected.
**
...
** is noble in the name of performance (_and 
** _not smashing the stack).
**
...
int mpc_parse_input(mpc_input_t *i, mpc_parser_t *init, mpc_result_t *final) {
  ...
  while (!mpc_stack_empty(stk)) {
    ...
    switch (p->type) {
      ...
      case MPC_TYPE_NOT:
        if (st == 0) { mpc_input_mark(i); MPC_CONTINUE(1, p->data._not.x); }
        if (st == 1) {
          if (mpc_stack_popr(stk, &r)) {
            mpc_input_rewind(i);
            p->data._not.dx(r.output);
            MPC_FAILURE(mpc_err_new(i->filename, i->state, "opposite", mpc_input_peekc(i)));
          } else {
            mpc_input_unmark(i);
            mpc_stack_err(stk, r.error);
            MPC_SUCCESS(p->data._not.lf());
          }
        }
      
      case MPC_TYPE_MAYBE:
        if (st == 0) { MPC_CONTINUE(1, p->data._not.x); }
        if (st == 1) {
          if (mpc_stack_popr(stk, &r)) {
            MPC_SUCCESS(r.output);
          } else {
      ...
      case MPC_TYPE_OR:
        ...
        if (p->data._or.n == 0) { MPC_SUCCESS(NULL); }
        
        if (st == 0) { MPC_CONTINUE(st+1, p->data._or.xs[st]); }
        if (st <= p->data._or.n) {
          if (mpc_stack_peekr(stk, &r)) {
            mpc_stack_popr(stk, &r);
          ...
          if (st <  p->data._or.n) { MPC_CONTINUE(st+1, p->data._or.xs[st]); }
          if (st == p->data._or.n) { MPC_FAILURE(mpc_stack_merger_err(stk, p->data._or.n)); }
        }
      
      case MPC_TYPE_AND:
        
        if (p->data._and.n == 0) { MPC_SUCCESS(p->data._and.f(0, NULL)); }
        
        if (st == 0) { mpc_input_mark(i); MPC_CONTINUE(st+1, p->data._and.xs[st]); }
        if (st <= p->data._and.n) {
          if (!mpc_stack_peekr(stk, &r)) {
            mpc_input_rewind(i);
            mpc_stack_popr(stk, &r);
            mpc_stack_popr_out(stk, st-1, p->data._and.dxs);
            MPC_FAILURE(r.error);
          }
          if (st <  p->data._and.n) { MPC_CONTINUE(st+1, p->data._and.xs[st]); }
          if (st == p->data._and.n) { mpc_input_unmark(i); MPC_SUCCESS(mpc_stack_merger_out(stk, p->data._and.n, p->data._and.f)); }
        }
...
static void mpc_undefine_or(mpc_parser_t *p) {
  ...
  int i;
  for (i = 0; i < p->data._or.n; i++) {
    mpc_undefine_unretained(p->data._or.xs[i], 0);
  }
  free(p->data._or.xs);
...
static void mpc_undefine_and(mpc_parser_t *p) {
  ...
  int i;
  for (i = 0; i < p->data._and.n; i++) {
    mpc_undefine_unretained(p->data._and.xs[i], 0);
  }
  free(p->data._and.xs);
  free(p->data._and.dxs);
...
static void mpc_undefine_unretained(mpc_parser_t *p, int force) {
  ...
  switch (p->type) {
    ...
    case MPC_TYPE_MAYBE:
    case MPC_TYPE_NOT:
      mpc_undefine_unretained(p->data._not.x, 0);
      break;
...
/*
** As `snprintf` is _not ANSI standard this 
...
mpc_parser_t *mpc_range(char s, char e) {
  ...
  p->data.range.x = s;
  p->data.range.y = e;
  return mpc_expectf(p, "character between '%c' _and '%c'", s, e);
}
...
mpc_parser_t *mpc_or(int n, ...) {
  ...
  p->type = MPC_TYPE_OR;
  p->data._or.n = n;
  p->data._or.xs = (mpc_parser_t **) malloc(sizeof(mpc_parser_t*) * n);
  
  va_start(va, n);  
  for (i = 0; i < n; i++) {
    p->data._or.xs[i] = va_arg(va, mpc_parser_t*);
  }
  va_end(va);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  p->data._and.n = n;
  p->data._and.f = f;
  p->data._and.xs = (mpc_parser_t **) malloc(sizeof(mpc_parser_t*) * n);
  p->data._and.dxs = (mpc_dtor_t *) malloc(sizeof(mpc_dtor_t) * (n-1));

  va_start(va, f);  
  for (i = 0; i < n; i++) {
    p->data._and.xs[i] = va_arg(va, mpc_parser_t*);
  }
  for (i = 0; i < (n-1); i++) {
    p->data._and.dxs[i] = va_arg(va, mpc_dtor_t);
  }  
  va_end(va);
...
** mpc constructs _and functions to
** parse the user regex string _and
** construct a parser from it.
**
** As it turns out lots of the standard
** mpc functions look a lot like `fold`
** functions _and so can be used indirectly
...
** builds an AST _and then traverses it
...
static void mpc_print_unretained(mpc_parser_t *p, int force) {
  ...
  if (p->type == MPC_TYPE_SINGLE) {
    ...
    s = (char *) mpcf_escape_new(buff, mpc_escape_input_c, mpc_escape_output_c);
    printf("'%s'", s);
    free(s);
  }
  ...
  if (p->type == MPC_TYPE_RANGE) {
    ...
    e = (char *) mpcf_escape_new(buff, mpc_escape_input_c, mpc_escape_output_c);
    printf("[%s-%s]", s, e);
    free(s);
    free(e);
  }
  ...
  if (p->type == MPC_TYPE_ONEOF) {
    s = (char *) mpcf_escape_new(p->data.string.x, mpc_escape_input_c, mpc_escape_output_c);
    printf("[%s]", s);
    free(s);
  }
  ...
  if (p->type == MPC_TYPE_NOT)   { mpc_print_unretained(p->data._not.x, 0); printf("!"); }
  if (p->type == MPC_TYPE_MAYBE) { mpc_print_unretained(p->data._not.x, 0); printf("?"); }
  ...
  if (p->type == MPC_TYPE_OR) {
    printf("(");
    for(i = 0; i < p->data._or.n-1; i++) {
      mpc_print_unretained(p->data._or.xs[i], 0);
      printf(" | ");
    }
    mpc_print_unretained(p->data._or.xs[p->data._or.n-1], 0);
    printf(")");
  }

  if (p->type == MPC_TYPE_AND) {
    printf("(");
    for(i = 0; i < p->data._and.n-1; i++) {
      mpc_print_unretained(p->data._and.xs[i], 0);
      printf(" ");
    }
    mpc_print_unretained(p->data._and.xs[p->data._and.n-1], 0);
    printf(")");
  }
...
/*
** These functions are slightly unwieldy _and
** also the whole of the testing suite for mpc
** mpc is pretty shaky.
**
** It could do with a lot more tests _and more
...
mpc_parser_t *mpca_and(int n, ...) {
  ...
  for (i = 0; i < (n-1); i++) {
    p->data._and.dxs[i] = (mpc_dtor_t)mpc_ast_delete;
  }    
  va_end(va);
...
** users to specify the grammar alone _and
...
** syntax _and spit out a parser that works.
**
** The grammar for this looks surprisingly
** like regex but the main difference is that
** it is now whitespace insensitive _and the
